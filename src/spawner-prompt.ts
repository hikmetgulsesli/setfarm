...
if (/^developer$/i.test(String(s.role || ""))) {
  lines.push("IMPLEMENT_LOOP=Edit scoped source, run CHECK_BUILD_CMD, run CHECK_TEST_CMD, validate evidence, then write required output. Once declared build/test pass, do not add optional new tests or probes; finish the claim.");
  lines.push("IMPLEMENT_DONE_FAST_PATH=After CHECK_BUILD_CMD and CHECK_TEST_CMD pass, run node .setfarm-bin/setfarm-evidence validate, write OUTPUT_CONTRACT fields, call step complete, then stop.");
