...
SETFARM_SUMMARY_TOOL_NODE
  echo "SUMMARY_IMPLEMENT_CONTEXT_CMD=CLAIM_SUMMARY_FILE='$CLAIM_SUMMARY_FILE' node .setfarm-bin/setfarm-summary implement-context"
  echo "SUMMARY_HELPER_RULE=Use SUMMARY_IMPLEMENT_CONTEXT_CMD as the primary handoff. Do not guess setfarm-summary topics, append pipes/redirection/chaining, cat helper scripts, or parse raw /tmp/claim JSON. Use RETRY_WORKTREE_PATCH_CMD/RETRY_SOURCE_SNAPSHOT_CMD only when those exact lines are printed."
...
if (/^developer$/i.test(String(s.role || ""))) {
  ...
  lines.push("SUMMARY_HELPER_RULE=Use SUMMARY_IMPLEMENT_CONTEXT_CMD as the primary handoff. Do not guess setfarm-summary topics, append pipes/redirection/chaining, cat helper scripts, or parse raw /tmp/claim JSON. Use RETRY_WORKTREE_PATCH_CMD/RETRY_SOURCE_SNAPSHOT_CMD only when those exact lines are printed.");
}
...
lines.push("DETAILS_RULE=Initial bootstrap output intentionally omits long story, retry, PR, and supervisor text. Use SUMMARY_IMPLEMENT_CONTEXT_CMD for the complete compact implement handoff; do not request all/full/story-brief helper topics.");
