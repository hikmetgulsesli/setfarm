...
export async function cleanupAbandonedSteps(advancePipeline: (runId: string) => Promise<{ advanced: boolean; runCompleted: boolean }> | { advanced: boolean; runCompleted: boolean }): Promise<void> {
  if (USE_PG) {
    // PG path: use EXTRACT(EPOCH FROM ...) instead of julianday
    ...
    for (const step of abandonedSteps) {
      ...
      if (newAbandonCount >= MAX_ABANDON_RESETS) {
        await pgRun("UPDATE steps SET status = 'failed', output = $1, abandoned_count = $2, updated_at = $3 WHERE id = $4", [singleDiagnostic, newAbandonCount, new Date().toISOString(), step.id]);
        await pgRun("UPDATE runs SET status = 'failed', updated_at = $1 WHERE id = $2", [new Date().toISOString(), step.run_id]);
        try { await pgRun("UPDATE claim_log SET outcome = 'abandoned', abandoned_at = $1, diagnostic = $2 WHERE run_id = $3 AND step_id = $4 AND outcome IS NULL", [new Date().toISOString(), singleDiagnostic, step.run_id, step.step_id]); } catch (e) { logger.warn("[cleanup] claim_log update failed: " + String(e), { runId: step.run_id }); }
        const wfId = await getWorkflowId(step.run_id);
        ...
        await pgRun("UPDATE steps SET status = 'pending', abandoned_count = $1, updated_at = $2 WHERE id = $3", [newAbandonCount, new Date().toISOString(), step.id]);
        try { await pgRun("UPDATE claim_log SET outcome = 'abandoned', abandoned_at = $1, diagnostic = $2 WHERE run_id = $3 AND step_id = $4 AND outcome IS NULL", [new Date().toISOString(), singleDiagnostic, step.run_id, step.step_id]); } catch (e) { logger.warn("[cleanup] claim_log update failed: " + String(e), { runId: step.run_id }); }
        emitEvent({ ts: new Date().toISOString(), event: "step.timeout", runId: step.run_id, workflowId: await getWorkflowId(step.run_id), stepId: step.step_id, detail: `Reset to pending — ${singleDiagnostic}` });
      }
    }
    ...
    for (const story of abandonedStories) {
      await pgRun("UPDATE stories SET status = 'pending', abandoned_count = abandoned_count + 1, updated_at = $1 WHERE id = $2", [new Date().toISOString(), story.id]);
    }
    ...
    for (const sv of stuckVerify) {
      try {
        ...
        if (lc.verifyEach && lc.verifyStep === sv.step_id) {
          await pgRun("UPDATE steps SET status = 'pending', updated_at = $1 WHERE id = $2", [new Date().toISOString(), sv.id]);
