export function preserveActionableStoryRetryOutput(currentOutput: string | null | undefined, diagnostic: string): string {
  ...
  const actionablePattern = /\b(?:PR_REVIEW_COMMENTS_OPEN|APP_INTEGRATION_[A-Z_]*REGRESSION|POST_MERGE_QUALITY_REGRESSION|VULNERABILITIES|SECURITY_FINDINGS?|STATUS:\s*retry)\b|actionable PR review comments/i;
  const infraPattern = /\bAGENT_(?:STEP|STORY)_STATE_MISMATCH\b|\bAGENT_PROCESS_(?:EXITED|TERMINAL|STUCK|HARD_STUCK)\b|\bAGENT_MODEL_TURN_STALLED\b|\bINFRA_RETRY\b|\bGUARDRAIL \[product-supervisor:implement\]|\bIMPLEMENT_NO_DELTA\b|\bNO_CHANGED_FILES\b/i;
