...
const BROWSER_INFRA = /(?:\b(agent-browser|browser control|playwright|chromium|chrome|page\.goto|browser|context|target page|webpreviewruntimedriver)\b[\s\S]{0,520}\b(executable doesn't exist|npx playwright install|ETIMEDOUT|ECONNREFUSED|ECONNRESET|EPIPE|timed out|timeout|target page|context or browser has been closed|browser has been closed|target closed|protocol error)\b|\bruntime did not become ready\b[\s\S]{0,220}\b(fetch failed|ECONNREFUSED|ECONNRESET|EPIPE|timed out|timeout)\b|\bpreview\b[\s\S]{0,220}\b(fetch failed|ECONNREFUSED|ECONNRESET|EPIPE|timed out|timeout|strictPort)\b|\bsystem smoke did not return structured JSON\b|\bsmoke did not return structured JSON\b)/i;
