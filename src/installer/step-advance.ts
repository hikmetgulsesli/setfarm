...
import { pgGet, pgQuery, pgRun, pgExec, pgBegin } from "../db-pg.js";
  ...
  if (USE_PG) {
    ...
    if (pendingStory) {
      ...
      if (loopStatus?.status !== STEP_STATUS.RUNNING) {
        await pgRun(
          "UPDATE steps SET status = 'pending', updated_at = $1 WHERE id = $2",
          [new Date().toISOString(), loopStepId]
        );
      }
      return { advanced: false, runCompleted: false };
    }
      ...
      if (lcForCheck.verifyEach && lcForCheck.verifyStep) {
        ...
        if (unverifiedStory) {
          await pgRun(
            ...
            [new Date().toISOString(), runId, lcForCheck.verifyStep]
          );
...
SUMMARY: ${verifiedCount}/${totalCount} stories verified, ${skippedCount} skipped, ${failedCount} failed`;
    ...
    await pgBegin(async (sql) => {
      await sql.unsafe(
        ...
        [loopSummaryOutput, new Date().toISOString(), loopStepId]
      );
...
VERIFICATION_SUMMARY: ${verifiedCount}/${totalCount} stories verified`;
          await sql.unsafe(
            ...
            [verifySummary, new Date().toISOString(), runId, lc.verifyStep]
          );
        }
      }
    });
