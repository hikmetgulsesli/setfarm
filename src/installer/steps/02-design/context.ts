...
const FIRST_ATTEMPT_REMINDER =
  ...
  "Each SCREEN_MAP entry needs screenId+name+type+description and should preserve surfaceIds when present. Keep metadata in English and visible copy in UI_LANGUAGE.";

export async function injectContext(ctx: ClaimContext): Promise<void> {
  // Design step needs PRD + repo + Product Surfaces for context. PRD is already
  // in context from plan step; preclaim performs Stitch generation and verify.
