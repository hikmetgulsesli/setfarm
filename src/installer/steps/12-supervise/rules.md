...
- Require English in all compiler-owned artifacts, source code, tests, fixtures,
  and visible application copy. `UI_LANGUAGE` is exactly English. Treat
  non-English PRD or Stitch text as raw evidence of a stale upstream artifact;
  block with the exact artifact instead of preserving or silently translating it.
