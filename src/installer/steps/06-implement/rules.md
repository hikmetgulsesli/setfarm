...
- Do not create debug/probe/source files with OpenClaw write/edit, including under `/tmp`. If you need a one-off runtime probe, use an inline `node -e`/shell command or an existing scoped test file; `/tmp/setfarm-progress-*.txt` is only for plain checkpoint notes.
...
- Evidence schemas are exact: `IMPLEMENT_INTENT.json` must use `"schema": "setfarm.implement-intent.v1"` and `IMPLEMENT_VERIFICATION_REQUEST.json` must use `"schema": "setfarm.implement-verification-request.v1"` with `"status": "ready_for_orchestrator_verification"`. Use `interactionRequests: []` and `runtimeEvidenceRequired.minFlowCount: 0` only for runtime/shell stories whose acceptance criteria do not require an interaction. If any interactive acceptance criterion cannot be executed by Setfarm, list it in `uncoveredCriteria` instead of silently claiming full coverage. Put only executable browser actions in `interactionRequests`; supported actions are `click`, `fill`, `press`, `wait`, `navigate`, and `snapshot`. Never put `load`, `verify`, `evaluate`, `assert`, build/test/source-grep/prose checks there.
- `interactionRequests` run from the app's initial loaded runtime state and execute in array order. Only request clicks for action IDs that are actually present in the rendered DOM or explicitly declared by the generated component type/screenUsageContract. Do not request a SCREEN_INDEX action when the generated component action type is `never`, screenUsageContract has no matching action ID, or scoped source does not render a matching `data-action-id`; put that criterion in `uncoveredCriteria` instead. Do not request a click on a control that is only visible after another screen/surface opens. First request or implement a reachable opener/navigation action, then request the target action. If the target surface cannot be reached from an allowed scoped change, put that criterion in `uncoveredCriteria` instead of claiming it tested.
