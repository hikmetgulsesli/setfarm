...
2. **ABSOLUTE SCOPE DISCIPLINE.** Write ONLY the files listed in SCOPE_FILES. That list is exhaustive for your story; every file the project needs was pre-planned into some story's scope.
   ...
   - If a generated screen file is only in SHARED_FILES, do not read the full file. Use `src/screens/SCREEN_INDEX.json`, `src/screens/index.ts`, COMPONENT REGISTRY, STORY_SCREENS, and UI BEHAVIOR CONTRACT for component/action contracts. Read full generated screen files only when they are listed in SCOPE_FILES, and then inspect one focused file at a time. This is machine-enforced by the Setfarm spawner: reading a generated `src/screens/*.tsx` file outside SCOPE_FILES kills and retries the claim before context overload.
