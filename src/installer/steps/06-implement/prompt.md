...
2. **ABSOLUTE SCOPE DISCIPLINE.** Write ONLY the files listed in SCOPE_FILES. That list is exhaustive for your story; every file the project needs was pre-planned into some story's scope.
   ...
   - Do not create debug/probe/source files with OpenClaw write/edit, including under `/tmp`. If you need a one-off runtime probe, use an inline `node -e`/shell command or an existing scoped test file. `/tmp/setfarm-progress-{{RUN_ID}}.txt` is only for plain checkpoint notes, not executable scripts.
   ...
   - Treat Stitch, DESIGN_DOM, UI_CONTRACT, and generated screen contracts as binding product design inputs, not optional inspiration. If generated `src/screens/*.tsx` files exist for your owned screen, use the injected contracts and owned generated component source. If no generated source exists for this stack, use the injected Stitch HTML/DESIGN_DOM/UI_CONTRACT and focused story-owned Stitch files to recreate the design in scoped files. Do not read unrelated cross-story design files.
   - UI_CONTRACT labels such as `notifications`, `help_outline`, `dashboard`, `settings`, `edit`, or `delete` may be icon ligature tokens from Stitch, not visible copy. Never render those token strings as source UI text or add Material Symbols/icon-font CSS. Use an inline SVG, an already-installed SVG icon component, or a short real product label where the control is not icon-only.
   ...
   - Use these exact implementation evidence schemas. Do not rename them, do not use slash-style schema IDs, and do not put build/source-grep checks in `interactionRequests`.
    ...
    `IMPLEMENT_VERIFICATION_REQUEST.json` minimal runtime/shell template:
     ...
     ```
     `interactionRequests: []` is allowed only when the story's acceptance criteria do not need a user/runtime interaction. For interactive criteria, request executable actions or list the unverified criteria in `uncoveredCriteria`; never leave interactive criteria as silently covered. `interactionRequests` is only for browser runtime actions that Setfarm can execute from the app's initial loaded state in array order. The only supported `action` values are `click`, `fill`, `press`, `wait`, `navigate`, and `snapshot`; never use `load`, `verify`, `evaluate`, `assert`, source-grep, build/test commands, or prose checks as interaction actions. Only target action IDs that are actually present in the rendered runtime DOM or explicitly declared by the generated component type/screenUsageContract. Do not request a SCREEN_INDEX action when the generated component action type is `never`, screenUsageContract has no matching action ID, or your scoped source does not render a matching `data-action-id`; put that criterion in `uncoveredCriteria` instead. If a target control appears only after opening another screen/surface, first request or implement a reachable opener action, then request the target action. For example: `{ "id": "open-settings", "action": "click", "target": "[data-action-id='settings-3']", "waitCondition": "dom_idle", "timeoutMs": 1000 }` followed by the settings action. Build, test, source-grep, and prose checks do not belong there; Setfarm already runs build/runtime capture separately.

3. **Binding design, scoped files.** The Stitch design may show elements that are not in your SCOPE_FILES (for example, a shared header while your scope is one screen component). Do not edit files another story owns. Inside your scoped files, however, the assigned Stitch screen is the source of truth: implement the visible structure, content hierarchy, navigation, cards/tables/forms/panels, spacing, states, and controls unless the story explicitly excludes them. If a generated screen component exists, import/render and wire it. If no generated component exists for this stack, recreate the assigned screen from Stitch HTML, DESIGN_DOM, UI_CONTRACT, and story screenshots/contracts in the owned files. Do not simplify the surface into a generic layout just because smoke tests would pass.
