...
function safeSnapshotPath(raw: string): string {
  ...
  if (WORKTREE_METADATA_PREFIXES.some((prefix) => file === prefix.slice(0, -1) || file.startsWith(prefix))) return "";
  return file;
}
...
function buildRetrySourceSnapshot(workdir: string, scopeFilesRaw: string, sharedFilesRaw: string, maxChars = 900_000): string {
  ...
  const scopeFiles = parseScopeFileList(scopeFilesRaw).map(safeSnapshotPath).filter(Boolean);
  const sharedFiles = parseScopeFileList(sharedFilesRaw).map(safeSnapshotPath).filter(Boolean);
  ...
  for (const file of scopeFiles) {
    ...
    if (snapshot.length + next.length > maxChars) {
...
export async function injectStoryContext(
  ...
  const retryPatchMemory = collectRetryWorktreePatchMemory(retryPatchRepoPath, context["story_workdir"] || "", story.storyId, [
    context["story_branch"],
    pipelineStoryBranch,
  ]);
