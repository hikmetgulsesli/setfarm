...
trigger label/icon + beklenen görünür davranış. PRD'de açıkça
yazmayan ama Stitch'te aktif görünen kontroller de ya gerçek route/panel/dialog açmalı ya da story'de
...
7. UI_BEHAVIOR_CONTRACT'teki her button/link/input için acceptanceCriteria yaz. Kriterler mekanik olmalı: `"<kontrol adı>" button opens the matching project-specific panel`, `"<kontrol adı>" navigates to its declared route`, `"<aksiyon adı>" changes the visible project state` gibi. Örnek isimleri kopyalama; PRD/Stitch içindeki gerçek kontrol adlarını kullan.
