...
interface SemanticSurfaceAction {
  id: string;
  surfaceId?: string;
  surfaceName?: string;
  controlHint?: string;
...
function contextPrdText(context?: Record<string, string>): string {
  return [
    context?.["prd"],
    context?.["PRD"],
...
function extractSemanticSurfaceActions(prdText: string): SemanticSurfaceAction[] {
  const actions: SemanticSurfaceAction[] = [];
  let currentSurfaceId = "";
  let currentSurfaceName = "";

  for (const line of String(prdText || "").split(/\r?\n/)) {
    const surface = line.match(/^\s*(?:#{1,6}\s*)?(?:SURFACE|Surface)\s*:?\s*(SURF_[A-Z0-9_]+)(?:\s*[-\u2013:]\s*(.+))?/i);
    if (surface) {
      currentSurfaceId = surface[1].toUpperCase();
      currentSurfaceName = (surface[2] || "").replace(/\*\*/g, "").trim();
      continue;
    }

    const name = line.match(/^\s*-\s*(?:\*\*)?Name(?:\*\*)?\s*:\s*(.+)$/i);
    if (name && currentSurfaceId) {
      currentSurfaceName = name[1].replace(/\*\*/g, "").trim();
      continue;
    }

    const permitted = line.match(/^\s*-\s*(?:\*\*)?(?:Permitted Actions|Actions)(?:\*\*)?\s*:\s*(.+)$/i);
    if (!permitted || !currentSurfaceId) continue;

    for (const item of splitActionItems(permitted[1])) {
...
function surfaceIdWords(surfaceId?: string): string[] {
  return normalizeKey(String(surfaceId || "").replace(/^SURF_/i, "").replace(/_/g, " "))
    .split(/\s+/)
    .filter((word) => word.length > 2);
}

function semanticActionMatchesScreen(action: SemanticSurfaceAction, screen: PredictedScreen): boolean {
  const title = normalizeKey(`${screen.screenId} ${screen.title} ${screen.filePath}`);
  const surfaceName = normalizeKey(action.surfaceName || "");
...
function semanticActionsForScreens(
  predicted: PredictedScreen[],
  screenIds: string[],
  actions: SemanticSurfaceAction[],
): SemanticSurfaceAction[] {
  if (actions.length === 0) return [];
  const screenSet = new Set(screenIds);
  const screens = predicted.filter((screen) => screenSet.has(screen.screenId));
  return actions.filter((action) => screens.some((screen) => semanticActionMatchesScreen(action, screen)));
}

function semanticActionVerb(actionId: string): string {
  return normalizeKey(actionId.replace(/^ACT_/i, "").replace(/_/g, " ")).split(/\s+/)[0] || "action";
}
...
function semanticActionMatchesRequirement(action: SemanticSurfaceAction, req: UiBehaviorRequirement): boolean {
  const key = requirementKey(req);
  const verb = semanticActionVerb(action.id);
  const hint = normalizeKey(action.controlHint || "");
...
function semanticStateChange(action: SemanticSurfaceAction): string {
  const key = normalizeKey(action.id);
...
function semanticUiFeedback(action: SemanticSurfaceAction): string {
  const key = normalizeKey(action.id);
...
function actionContractsForScreens(
  repo: string,
  screenIds: string[],
  semanticActions: SemanticSurfaceAction[] = [],
): StoryImplementationContract["owned_actions"] {
  const screenSet = new Set(screenIds);
...
function screenImplementationContract(params: {
  ...
  const ownedActions = actionContractsForScreens(params.repo, params.screenIds, params.semanticActions || []);
  return {
...
export function buildAutoStoriesOutput(params: {
  ...
  const semanticSurfaceActions = extractSemanticSurfaceActions(contextPrdText(params.context));
  ...
  if (maxStories === 1) {
    ...
    stories = [{
      ...
      implementation_contract: screenImplementationContract({
        ...
        screenIds,
        screenFiles,
        semanticActions: semanticActionsForScreens(predicted, screenIds, semanticSurfaceActions),
      }),
    }];
  } else {
    ...
    groups.forEach((group, index) => {
      ...
      stories.push({
        ...
        implementation_contract: screenImplementationContract({
          ...
          semanticActions: semanticActionsForScreens(predicted, groupScreenIds, semanticSurfaceActions),
        }),
      });
    });
  }
