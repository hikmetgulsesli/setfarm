...
const PATTERNS: Array<{ pattern: RegExp; category: ErrorCategory; suggestion: string }> = [
  ...
  { pattern: /^MASKED_CHECK_COMMAND:/i, category: "MASKED_CHECK_COMMAND", suggestion: "The worker ran build/test through an output-filtering pipeline that can hide the real exit code. Rerun the declared build/test command without a pipe, or redirect output to a log and inspect it only after preserving the command exit status." },
  ...
  { pattern: /(?:IMPLEMENT_EVIDENCE_INCOMPLETE|IMPLEMENT_INTERACTION_FAILED)[\s\S]{0,2200}\b(?:missingTargetActionId=|target is not present in the current runtime surface|availableActionIds=|suggestedActionIds=|currentScreen=)/i, category: "UI_INTERACTION_TARGET_UNREACHABLE", suggestion: "Runtime evidence requested a target that is not reachable from the current runtime surface. Keep IMPLEMENT_* schemas strict; if feedback lists availableActionIds or suggestedActionIds, use one of those exact DOM ids in the verification request instead of inventing a similar id. Otherwise fix scoped app/shell wiring so interactions run from the initial loaded state in order: first expose or click a reachable opener action that opens the target surface, then request the target action. If no reachable path exists in scope, list the criterion in uncoveredCriteria instead of claiming it covered." },
  ...
  { pattern: /(?:^|\n)[^\n]*\bGENERATED_SCREEN_SHARED_READ:/i, category: "GENERATED_SCREEN_SHARED_READ", suggestion: "Use claim-summary designContracts, SCREEN_INDEX.json, and src/screens/index.ts for shared generated screens. Do not use the OpenClaw read tool or shell commands to read forbidden src/screens/*.tsx files outside scopeFiles." },
  { pattern: /(?:^|\n)[^\n]*\bGENERATED_SCREEN_NOT_INTEGRATED:/i, category: "GENERATED_SCREEN_NOT_INTEGRATED", suggestion: "Render every owned generated screen through the app/router surface and wire its declared actions prop IDs before reporting done. Preserve previous-story behavior while replacing duplicate custom UI." },
  { pattern: /(?:^|\n)[^\n]*\bGENERATED_SCREEN_REQUIRED_PROPS_UNWIRED:/i, category: "GENERATED_SCREEN_REQUIRED_PROPS_UNWIRED", suggestion: "Wire every required generated screen prop from scoped app state/adapters before reporting done. Preserve generated component prop contracts when editing app/router/shared files." },
  ...
  { pattern: /(?:^|\n)[^\n]*\bGENERATED_SCREEN_REGRESSION:/i, category: "GENERATED_SCREEN_REGRESSION", suggestion: "Preserve every previously verified generated screen route/rendering surface while adding the current story screens. Do not replace previous generated screens with custom duplicate UI." },
  { pattern: /(?:^|\n)[^\n]*\bAPP_INTEGRATION_(?:SCOPE|SEMANTIC|PROP)_REGRESSION:/i, category: "APP_INTEGRATION_REGRESSION", suggestion: "Restore the previously accepted app/router wiring from the current story branch base before adding this story. Preserve prior story action helpers, keyboard/control bridges, data-testid values, ARIA/live-region/status contracts, and generated screen props; then apply only the current story's scoped additions." },
  { pattern: /(?:^|\n)[^\n]*\bGENERATED_SCREEN_SHELL_CHROME_UNSAFE:/i, category: "GENERATED_SCREEN_SHELL_CHROME_UNSAFE", suggestion: "Remove visible diagnostic/session/status/debug/QA chrome around generated full-screen Stitch screens. Keep smoke/debug state in window.app/globalThis.app or test-only data, and keep generated screens mounted as the visual viewport root on mobile and desktop." },
  { pattern: /(?:^|\n)[^\n]*\bGENERATED_SCREEN_SHELL_LANDMARK_UNSAFE:/i, category: "GENERATED_SCREEN_SHELL_LANDMARK_UNSAFE", suggestion: "Remove app-shell semantic landmark wrappers around generated full-screen Stitch screens. Generated screens own their main landmark; use a neutral div data-setfarm-root container for app state wiring." },
  { pattern: /(?:^|\n)[^\n]*\bGENERATED_SCREEN_LAYOUT_MOUNT_UNSAFE:/i, category: "GENERATED_SCREEN_LAYOUT_MOUNT_UNSAFE", suggestion: "Generated full-screen screens with sibling sidebar/content layouts must be mounted inside a neutral flex data-setfarm-root viewport container. Prefer a combined root such as className=\"relative flex min-h-screen w-full overflow-hidden\" when generated screens also use absolute/fixed viewport layers. Preserve generated screen structure and fix the app/root wrapper instead of rewriting generated screens." },
  { pattern: /(?:^|\n)[^\n]*\bGENERATED_SCREEN_VIEWPORT_MOUNT_UNSAFE:/i, category: "GENERATED_SCREEN_VIEWPORT_MOUNT_UNSAFE", suggestion: "Generated absolute/fixed full-screen screens must be mounted inside a neutral positioned viewport root. Prefer a combined root such as className=\"relative flex min-h-screen w-full overflow-hidden\" so sibling sidebar/content generated screens also render side-by-side. Fix the app/root wrapper instead of rewriting generated screens." },
