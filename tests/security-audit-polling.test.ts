...
const WORKFLOW_DIR = path.resolve(import.meta.dirname, "..", "workflows", "security-audit");
...
describe("security-audit workflow polling config", () => {
  ...
  it("still has all expected agents", async () => {
    ...
    assert.deepEqual(ids, ["scanner", "prioritizer", "setup", "fixer", "verifier", "tester", "pr"]);
  });

  it("still has all expected steps", async () => {
    const spec = await loadWorkflowSpec(WORKFLOW_DIR);
    const stepIds = spec.steps.map((s) => s.id);
    assert.deepEqual(stepIds, ["scan", "prioritize", "setup", "fix", "verify", "test", "pr"]);
  });
