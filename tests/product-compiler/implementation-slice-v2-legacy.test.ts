...
import {
  LegacyImplementationSliceV2Schema as ImplementationSliceV2Schema,
...
import {
  ImplementationSliceV2Schema as CanonicalImplementationSliceV2Schema,
...
} from "../../src/product-compiler/slice-compiler-v2-legacy.js";
import {
  CONTAINED_GAME_TASK,
...
describe("ImplementationSliceV2 exact authority compiler", { concurrency: 1 }, () => {
  it("compiles one deterministic Stitch story closure with exact controls, inputs, files, build, runtime, and source closure", async () => {
    ...
    assert.equal(
      CanonicalImplementationSliceV2Schema.safeParse(first.slice).success,
      false,
    );
