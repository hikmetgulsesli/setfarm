...
function observe(home: string, texts: string[], fault = "", census?: string, defaultAction?: string): any {
  ...
  if (census !== undefined || defaultAction !== undefined) {
    ...
    fs.writeFileSync(transportFile, `export async function observeLegacyDatabaseCensusV1(url,cold,profile){${census ?? "globalThis.dbCalls++;return Object.freeze({activeRunCount:0})"}}
      ...
      export async function observeLegacyDatabaseCensusAndBindingRowsV6(url){
        globalThis.v6Calls++;
        if(url!=='postgresql://fixture:PG_SENTINEL@localhost/setfarm')throw Error('WRONG_V6_URL');
        await Promise.resolve();globalThis.combinedDrift?.();
        if(globalThis.combinedFailure)throw Error('PG_SENTINEL_PRIVATE_DATABASE_FAILURE');
      ...
      export async function observePositiveWorktreeActiveBindingCutoverDatabaseV1(url){
        ...
        await Promise.resolve();globalThis.combinedDrift?.();
        if(globalThis.combinedFailure)throw Error('PG_SENTINEL_PRIVATE_DATABASE_FAILURE');
        return Object.freeze({schema:'fixture-active-binding-v1'});
      }`);
    source = source.replace(marker, `await import(${JSON.stringify(pathToFileURL(transportFile).href)})`);
    source = source.replace('await import("./baseline-positive-worktree-active-binding-cutover-database-v1.js")',
      `await import(${JSON.stringify(pathToFileURL(transportFile).href)})`);
...
test("held V6 binding census requires qualification, uses one private URL and closes", () => defaultFixture((home, texts) => {
  const early = observe(home, texts,
    `evidence=()=>({v6Calls:globalThis.v6Calls,nodeCloses:globalThis.nodeCloses});`,
    undefined, `await context.censusAndBindingRows();return context.observation;`);
  assert.match(early.error, /DEPLOYMENT_CUTOVER_LAUNCHER_OBSERVATION_INVALID/);
  assert.deepEqual(early.evidence, { v6Calls: 0, nodeCloses: 2 });
  const qualified = observe(home, texts,
    `evidence=()=>({v6Calls:globalThis.v6Calls,v5Calls:globalThis.v5Calls,
      nodeCloses:globalThis.nodeCloses});`, undefined,
    `await context.qualifyPassiveHome();const combined=await context.censusAndBindingRows();
      return Object.freeze({combined,configuration:context.observation});`);
...
test("held non-pre32 active binding snapshot requires qualification and keeps the URL private", () => defaultFixture((home, texts) => {
  const early = observe(home, texts,
    `evidence=()=>({activeBindingCalls:globalThis.activeBindingCalls,nodeCloses:globalThis.nodeCloses});`,
    undefined, `await context.activeBindingSnapshot();return context.observation;`);
  assert.match(early.error, /DEPLOYMENT_CUTOVER_LAUNCHER_OBSERVATION_INVALID/);
  assert.deepEqual(early.evidence, { activeBindingCalls: 0, nodeCloses: 2 });
  const qualified = observe(home, texts,
    `evidence=()=>({activeBindingCalls:globalThis.activeBindingCalls,v6Calls:globalThis.v6Calls,
      nodeCloses:globalThis.nodeCloses});`, undefined,
    `await context.qualifyPassiveHome();const combined=await context.activeBindingSnapshot();
      return Object.freeze({combined,configuration:context.observation});`);
  assert.equal(qualified.observation?.combined.schema, "fixture-active-binding-v1", JSON.stringify(qualified));
  assert.deepEqual(qualified.evidence, { activeBindingCalls: 1, v6Calls: 0, nodeCloses: 2 });
  assert.equal(qualified.frozen, true);
  assert.doesNotMatch(JSON.stringify({ early, qualified }), /PG_SENTINEL|TOKEN_SENTINEL|SocketSentinel/);
}));
