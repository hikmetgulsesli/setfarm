...
const WORKFLOW_DIR = path.resolve(import.meta.dirname, "..", "workflows", "bug-fix");
...
describe("bug-fix workflow polling config", () => {
  ...
  it("still has all expected agents", async () => {
    ...
    assert.deepEqual(ids, ["triager", "investigator", "setup", "fixer", "verifier", "pr"]);
  });

  it("still has all expected steps", async () => {
    const spec = await loadWorkflowSpec(WORKFLOW_DIR);
    const stepIds = spec.steps.map((s) => s.id);
    assert.deepEqual(stepIds, ["triage", "investigate", "setup", "fix", "verify", "pr"]);
  });
