...
The pinned absolute-path checks detect stable drift but do not make path creation race-proof against a same-UID actor swapping an ancestor between checks. A same-UID actor could also forge both hardlinks; their structure alone does not authenticate the fsync path. This store therefore cannot establish adversarial filesystem confinement or exclusive cutover ownership on its own; the future effect controller and host exclusion proof must retain that uncertainty.
