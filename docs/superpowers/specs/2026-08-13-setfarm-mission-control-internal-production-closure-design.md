...
Activation follows only after entry: strict A body parser/hash/ref in import-inert owner core; new A creation from current operation/PBA/source; historical A replay from embedded evidence and Git ancestry without current equality; generic PostgreSQL store/activator/current in `src/db-pg.ts`; then the zero-input A wrapper. Generic failure classification is the typed closed union `SUPERSEDED|CORRUPTION`. The sole strict canonical activation status union has one common exact key set and only `absent|active|blocked`: absent has every activation/manifest/source member null and null reason; active has exact successor activation/head, receipt, manifest, source-build and status pairs non-null, null predecessor pairs/reason; blocked exposes no partial activation pair and has exactly one finite non-null `SUPERSEDED|CORRUPTION|CURRENT_ENTRY_UNAVAILABLE|CURRENT_SOURCE_DRIFT` reason. Wrapper receipt hash is exactly `hashCanonicalJson({schema,plan,manifestHash,sourceBuildAuthorityRef,sourceBuildAuthorityHash,predecessorActivationRef,predecessorActivationHash,predecessorHeadRef,predecessorHeadHash,successorActivationRef,successorActivationHash,successorHeadRef,successorHeadHash})`; receipt ref is `setfarm://internal-production/baseline-owner-producer-manifest-activation-receipt/sha256/<receiptHash>`. Status ref/hash cover every declared member except themselves. Task 7 jq checks the exhaustive sorted key set and every active relation on both observations, and requires each terminal ref suffix to equal its adjacent hash under the exact receipt, activation, head, A-source, and status domain prefixes rather than generic `startswith` grammar.
...
- Deliver one import-inert owner-admission core with exactly 35 categories, 35 census-map keys, and 36 covered scalar counters; freeze the exact sixteen-row A manifest and `16/26/32/48/57` cumulative phase counts; enumerate every production birth/terminal/direct-SQL path through the literal 109-path File Map and fail-closed AST inventory; add OA17's terminal build-output tree, stable controller-build projection, zero-input clean-source observer, and historical Git replay plus OA18's ledger/retention paths; add the exact four-relation PostgreSQL activation authority, strict A source-build/PBA/vendor cross-binding, delivered-phase registry, same-transaction predecessor/source/activation/head/current CAS, and transaction-pinned current resolver; keep production repository/controller/fixed resolver composition solely in `src/db-pg.ts`, and expose only pair-input idempotent resolution/close.
