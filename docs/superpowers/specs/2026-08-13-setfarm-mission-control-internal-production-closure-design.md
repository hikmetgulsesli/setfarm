...
- Deliver one import-inert owner-admission core with exactly 35 categories, 35 census-map keys, and 36 covered scalar counters; freeze the exact sixteen-row A manifest and `16/26/32/48/57` cumulative phase counts; enumerate every production birth/terminal/direct-SQL path through the literal 101-path File Map and fail-closed AST inventory; add same-transaction reservation/close resolution, keep production repository/controller/fixed terminal-resolver composition solely in `src/db-pg.ts`, and expose only pair-input idempotent close/resolution.
