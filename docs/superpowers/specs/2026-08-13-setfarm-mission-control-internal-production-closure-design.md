...
The successor is registry ordinal 32 and the sole guarded migration class member. Generic planning must expose it as pending, generic apply must skip it, and generic verify must remain fail-closed/pending until Task 7's exact purpose-bound operation applies it. A targeted v31 audit and one zero-input guarded-successor inspector are the only pre-Task7 database authorities. The applied receipt hashes both causal predecessor pairs—v31 audit ref/hash and pending-successor ref/hash—and every current/post-rebind, restart, Task 8, and B–E consumer reopens them through that receipt/successor chain. Migration 32 also creates the PostgreSQL owner-reservation sidecar and singleton owner-admission-head relation. Begin/adopt writes the pending reservation; canonical owner insertion and sidecar binding commit in the same PostgreSQL transaction; close is category-typed and idempotent. This is database atomicity only and makes no claim about filesystem, process, listener, or service-dispatch atomicity.

Task 0 remains source-only. It may produce reviewed source/tests and deterministic checked-in contract/digest artifacts, and its focused tests may use an isolated private migration-32 test path. It must not apply 32 to the live database, create live reservations or authorities, start a canary, restart/rebind a service, or write a backup/receipt/runtime store. The local PBA wire parser is strict over the fully enumerated all-required/non-null nested evidence body, eight ordered path identities, focused-test receipt, and ordered twelve-entry vendor-lock projection; inventory remains 10 → 12 → 14. A's restart helper uses only the three literal launchd labels and code-owned UID/argv, and its A-owned seven-entry forward D identity registry records module path/export identity without importing D.
...
- Deliver the exact guarded-migration-32 class/API, v31 targeted audit, sole pending-successor inspector, source-integrity/digest binding, and private isolated-test mode; generic apply never consumes guarded work and generic verify remains pending until Task 7.
- Deliver one import-inert owner-admission core with exactly 35 categories, 35 census-map keys, and 36 covered scalar counters; bind every covered PostgreSQL owner through the migration-32 sidecar in the same transaction and use typed idempotent close.
