...
The current-entry pair is deliberately the pre-full-rebind predecessor. Task 6A first records an applied migration receipt that carries the exact read-only v31 audit pair and pre-apply pending-successor projection pair as its four causal predecessor fields, then records the successor-current audit, A-manifest activation, pre-canary spawner restart, mixed loaded-runtime authority, and canary chain. Generic full migration verification is invalid only while guarded migration 32 remains pending; it succeeds after Task 6A's exact purpose-bound application. Task 7 consumes the already-applied/current migration receipt and current-entry pair, performs no schema mutation, rebuilds and rebinds all scoped services, then seals a strict `InternalProductionPostRebindEntryAuthorityV1` successor after fresh source/build/schema/service/PBA-evidence and complete zero-owner verification. Task 8 and every B/C/D/E campaign, preflight, status, coordination, recovery, fleet, and closure authority consume the post-rebind pair; they never require the pre-full-rebind pair to remain current after Task 7's authorized service mutations. A failure at either gate remains a blocker; the rebaseline does not weaken, bypass, or reinterpret any runtime, migration, or acceptance guard.

The successor is registry ordinal 32 and the sole guarded migration class member; one ordered migration creates both bootstrap-main-claim handoff schema and the PostgreSQL owner-reservation sidecar/singleton owner-admission-head schema. Generic planning exposes it as pending, generic apply skips it, and generic verify remains fail-closed/pending until Task 6A's exact purpose-bound operation applies it under a fresh v31 audit and complete-zero-owner guard. A targeted v31 audit and one zero-input guarded-successor inspector are the only pre-apply database authorities. The applied receipt hashes both causal predecessor pairs—v31 audit ref/hash and pending-successor ref/hash—and every current/post-rebind, restart, Task 8, and B–E consumer reopens them through that receipt/successor chain. The isolated test lifecycle is fixed as generic automatic apply, then one zero-argument test-only capability for migration 32, then generic full verify; the capability has no production import, environment toggle, caller-selected ID, body, database, or evidence. The import-inert owner core receives an explicit `InternalProductionOwnerAdmissionRepositoryV1`, controller port, category-specific authenticated terminal-resolver registry, and the same passed `PgTransactionSql` for begin/adopt, canonical owner insertion, and sidecar bind. Public close accepts only `{reservationRef,reservationHash,terminalAuthorityRef,terminalAuthorityHash}`; the controller derives category and terminal authority internally, and repository compare-and-swap/retry semantics return the byte-identical close or fail closed on conflict. This is database atomicity only and makes no claim about filesystem, process, listener, or service-dispatch atomicity.

Task 0 remains source-only. It may produce reviewed source/tests and deterministic checked-in contract/digest artifacts, and its focused tests may use the isolated private migration-32 lifecycle above. It must not apply 32 to the live database, create live reservations or authorities, start a canary, restart/rebind a service, or write a backup/receipt/runtime store. The local PBA wire parser is strict over the fully enumerated all-required/non-null nested evidence body, eight ordered path identities, focused-test receipt, and ordered twelve-entry vendor-lock projection; inventory remains 10 → 12 → 14. `producerCommit`, `deliveryMergeSha`, `currentSource.sha`, and `currentSource.treeHash` accept only 40- or 64-character lowercase Git object hashes; every content/blob/build/receipt/lock/evidence hash is exactly 64 lowercase hexadecimal characters, with boundary fixtures proving the distinction. A's restart surface first prepares a code-owned authorization from only a closed service enum and returns its pair, then consumes only that pair and derives the service internally; the helper uses only the three literal launchd labels and code-owned UID/argv. A's seven-entry forward D identity registry records exact module path/export identity without importing D.
...
Treat the delivered Mission Control Product Build Authority V2 behavior and operational-snapshot-v3 work as immutable baseline inputs, add only the constructible read-only PBA V2 delivery-evidence projection, reopen and verify the delivered PR #86 Authority-V3/v31 authority separately from Task 0's sole guarded bootstrap successor, have Task 6A apply that successor before A-manifest activation and a spawner-only rebind/canary, seal the strict current-entry predecessor, then have Task 7 seal the strict post-rebind successor after read-only schema validation and full service rebuild/rebind.
...
- Deliver one import-inert owner-admission core with exactly 35 categories, 35 census-map keys, and 36 covered scalar counters; inject its exact repository/controller/resolver ports, pass one `PgTransactionSql` through begin/adopt plus owner insert/bind, and expose only pair-input category-resolved idempotent close.
...
- Bind current Task 0 controller source/tree/build separately from delivered loaded-runtime service authority without requiring equality; after Task 6A applies migration 32 and activates A's manifest, rebind only the spawner to Task 0, run the canary through its source-run fence, and leave the dashboard/Mission Control on delivered generations until Task 7.
...
- Reopen the exact v31 historical/current-authority pair and pending-successor projection before Task 6A's guarded apply; record both in the applied receipt, then require generic full migration verification and a current audit before A-manifest activation, spawner-only rebind, or fresh canary.
