...
Before the first loaded-process or `launchctl` observation, the retention-operation core current-observes and freezes OA17 `sourceBuild`, then the exact PBA V2 pair/body, then constructs the exact expected-runtime tuple in order spawner, dashboard, Mission Control. Spawner and dashboard reuse one identical pair/body: pair `{sourceSha:sourceBuild.sha,sourceTreeHash:sourceBuild.treeHash,controllerBuildHash:sourceBuild.buildHash}`, body byte-equal to operation `sourceBuild`. Mission Control reuses the operation's exact `productBuildAuthorityV2DeliveryEvidence` pair and `productBuildAuthorityV2Observation` body, and the pair also equals that body's response pair. Historical recovery takes these embedded values through strict operation parsing and the pure PBA response parser, never from a loaded process, today's source CLI, or today's PBA CLI. This is a projection of existing OA17/PBA authority, not a new producer, store, resolver, ref, or hash domain. Only after that freeze does fixed no-shell `/bin/launchctl print gui/<uid>/<label>` run under literal PATH/C-locale environment and the same 10-second/1-MiB bounds.

Loaded proof records the explicit-or-derived Program, arguments, working directory, exact environment-key tuple, PID, frozen `/bin/ps` start/group identity, and code-owned actual executable/entrypoint/source/tree/build/generation observation. Setfarm actual source SHA/tree/build equals the operation OA17 body; Mission Control actual source SHA/tree/build equals `productBuildAuthorityV2Observation.response.evidence.currentSource`. `expectedObservedFieldEqualityHash` covers exact `{schema:"setfarm.platform-build-generation-expected-observed-field-equality.v1",label,expectedRuntimeSource,loadedProcessObservation}` and is recomputed. Executable/entrypoint/generation remain independent code-authenticated actual facts and cannot generate expected authority. Every effective-Program/argv/working-directory/environment path token uses an at-most-32-hop, loop-free, identity-stable, no-follow symlink-chain resolution against independently established effective working directory. Every executable/entrypoint/resolved path must be outside candidate/quarantine by lexical, canonical realpath, and device/inode ancestry; crossed/missing expected tuple entries, a service not on its expected body, or unverifiable generation blocks and requires rebind.

Environment names are exact ordered tuples: spawner `[PATH,SETFARM_PG_URL]`; dashboard `[PATH,SETFARM_OPERATIONAL_WRITE_TOKEN,SETFARM_PG_URL]`; Mission Control `[CLI_PATH,MC_HOST,MC_INTERNAL_URL,MC_PORT,PATH,PROJECTS_DIR,PROJECTS_JSON,SETFARM_DIR,SETFARM_OPERATIONAL_WRITE_TOKEN,SETFARM_PG_URL,SETFARM_REPO_DIR,SETFARM_URL]`. Missing, duplicate, reordered, extra, or unknown keys block. Grammars/classifications are exact: PATH is colon path-list; CLI_PATH/PROJECTS_DIR/SETFARM_DIR/SETFARM_REPO_DIR are absolute paths; PROJECTS_JSON is an absolute file path; MC_INTERNAL_URL/SETFARM_URL are absolute HTTP URLs; MC_HOST is a host scalar; MC_PORT is a decimal-port scalar; SETFARM_OPERATIONAL_WRITE_TOKEN is an opaque-secret scalar; SETFARM_PG_URL is a PostgreSQL connection URL. PATH/path/URL fields use exact structural tokenization. Host/port/token/database values are secret-redacted, retain the exact raw-value hash, and undergo deterministic conservative path/URL token scan; the database URL additionally uses structural URL components. Every environment value is persisted only as the discriminated redacted/raw-null union with `valueHash===rawValueHash`, exact name/source/ordinal, grammar/classification/tokenization/scan policy, and never raw secret bytes.
...
Treat the delivered Mission Control Product Build Authority V2 behavior and operational-snapshot-v3 work as immutable baseline inputs, add only the constructible read-only PBA V2 delivery-evidence projection, freshly publish the exact OA18 v31/pending current records from current source/DB evidence, then have one prepared Task 6A operation perform pre-schema spawner sealing, guarded-32 application, A activation, same-generation admission readiness, and the canary before Task 7's read-only schema validation/full rebind.
