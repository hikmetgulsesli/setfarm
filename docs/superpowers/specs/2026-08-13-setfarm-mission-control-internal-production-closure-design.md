...
- Freeze one named-field service census, one strict nested current-entry status wire body, and one strict current-entry verification wire body; reject service arrays, flattened lifecycle mirrors, undeclared status fields, structural clones, and OpenClaw source/build claims.
