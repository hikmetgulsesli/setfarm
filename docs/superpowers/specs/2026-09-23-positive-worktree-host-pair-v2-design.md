...
The fixture seam rejects a physical observer that does not call the callback exactly once, calls it twice, or resolves before the callback settles, and rejects malformed top-level catalog/snapshot labels or hashes. The seam can observe early return, not prove the fixture observer's syntactic `await`; production timing relies on the trusted catalog implementation's explicit `await betweenPasses()`. A faulty fixture observer may abandon the callback, so its eventual database rejection must remain handled even after the pair refuses early; it must never become an unhandled process rejection. Database or physical observer failure rejects the pair; it never substitutes empty rows or a successful catalog. The production catalog's own strict held checks, blocker semantics, and cleanup remain authoritative and unchanged.
