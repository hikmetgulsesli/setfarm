...
10. In receipt and CLI tests, deliver the already predeclared finite source-bootstrap lifecycle and exact three verbs. Require the fixed pending record before any fence/reservation byte, pending-input/prepared/terminal visibility-head ordering, atomic acquire, intent/outbox/operation, one dispatch/run, reciprocal binding, both terminal authorities, compound close, pair-close publication, release, receipt, and terminal visibility. Crash before/after every record, fsync, CAS, dispatch acknowledgement, database commit, and response; a fresh zero-input resume adopts the same operation/run and a crossed/one-sided/ambiguous prefix returns only `RECOVERY_SOURCE_BOOTSTRAP_PREFIX_AMBIGUOUS` without mutation. Assert exact phase nullability, no second start, no caller override, generic reservation/compound-close domains, runtime/store/database pre-manifest zero, and removal of permanent textual absence only for the two private db-pg producers.

Each group is captured RED before implementation, made GREEN by the smallest exact18 source/test change, then rerun in the exact eight-file Task 12 direct matrix and independently reviewed. Documentation delivery itself remains source-only: no build, live database, service, socket, process, current-entry store, authority, canary, run, staging, commit, or push mutation belongs to this amendment step.
