...
- Create through one Setfarm-owned source claim before live mutation:
  ...
  - `setfarm/src/internal-production/baseline-owner-producer-manifest-activation-controller-v1.ts` — A-only, import-inert, path-free zero-input controller that cross-binds the current-entry/PBA/clean-Setfarm source authority, invokes the PostgreSQL generic activation for the exact sixteen-row A manifest, and derives its wrapper receipt/status from committed activation/head/current rows.
  ...
  - `setfarm/tests/internal-production/baseline-post-handoff-receipt-v1.test.ts` — includes the exact named-field service census, pre-mutation runtime authority, sole status/verification wire shapes, twelve-state/nested-phase strict-nullability, store, pair-resolution, crash-prefix, clone, count/source/listener drift, flattened-field rejection, and OpenClaw-null-source tests.
  - `setfarm/tests/internal-production/baseline-owner-producer-manifest-activation-controller-v1.test.ts` — A controller source/PBA/vendor cross-binding, PostgreSQL receipt/status, concurrency, interruption, response-loss replay, CLI, import-inertness, and no-producer-before-activation tests.
...
`baseline-owner-producer-manifest-activation-controller-v1.ts` remains a separate A-only zero-input wrapper. It reopens the fixed current-entry operation and PBA pair, derives the strict A source-build pair through the fixed A resolver, and calls the generic activator only as `{expectedPredecessor:null,manifests:[INTERNAL_PRODUCTION_OWNER_PRODUCER_MANIFEST_A_V1],orderedSourceBuildAuthorities:[aPair]}`. It accepts no manifest/plan/phase/pair/body/root/store/SQL/resolver input. A response-loss retry reopens current and adopts only the identical A activation; a different non-null current, manifest/source drift, or future phase blocks without mutation. The wrapper receipt/status are derived from the committed PostgreSQL activation/current rows; status is exactly `absent | active | blocked`, with no externally observable `activating` state, filesystem operation, receipt, head, or mutable status locator.

The two focused activation suites use test-first RED/GREEN coverage for: exact four-relation DDL/projector/digest and internal `existingRelations` four-to-eight projection; literal identifier length/name and rejection of PostgreSQL truncation; every literal CHECK/FK/index/function/trigger definition via normalized catalog output; arbitrary current UPDATE rejection; canonical TEXT versus JSONB metadata and whitespace/key-order/numeric/duplicate-key drift; absent/duplicate/half-null singleton current and exact all-null-seed `null`; initial A activation; source resolution before target classification; shared/prior source presence without false partial-target failure; same-target adoption before predecessor validation; exact-target/current-descendant typed superseded/stale; exact response-loss adoption; one-target/mismatched-target/unrelated-or-broken-current corruption; concurrent identical initial activation; conflicting initial activation; two appenders from one predecessor; stale/mixed/half-null predecessor; PostgreSQL transaction rollback after source, activation, or head insert and before/after current CAS/commit; current CAS loss; transaction-lock blocking and reopen; pinned-current equality during owner admission; immutable-row UPDATE/DELETE/TRUNCATE; source canonical-byte tamper; stable A–E pair envelope with A-only body registry; source pair/plan swap; new-A current operation/source/PBA/vendor/ancestor mismatch; historical A resolution after Setfarm or Mission Control main/build advances; missing historical Git object or invalid embedded PBA/operation/ancestor proof; dirty/stale new Setfarm source; registry/census ordered-array reorder or projection drift; activation/head/ref/hash tamper; broken predecessor chain/fork; undeclared/future/skipped phase; empty B–E body/union registration; and caller resolver/repository/body injection. Source/transcript fixtures still prove guarded migration, A activation, admission-ready, and first producer ordering.
