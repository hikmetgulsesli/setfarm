...
- Create through one Setfarm-owned source claim before live mutation:
  ...
  - `setfarm/src/internal-production/baseline-owner-producer-manifest-activation-controller-v1.ts` — A-only, import-inert, path-free controller that durably activates the exact sixteen-row A manifest and seals the predecessor/successor activation-head wrapper receipt.
