...
2. [ ] Write RED tests for strict held candidate join, unresolved cases, retained roots, callback/cardinality and second-pass mismatch, bootstrap authority/hash/argv refusal.
3. [ ] Implement pure bounded join, separate active-binding held pair, zero-input launcher flow and authenticated verb without changing V1 outputs.
4. [ ] Run focused and proportional suites, contracts, independent read-only review, then commit/push reviewed PR. Resolve all evidence-backed findings.
