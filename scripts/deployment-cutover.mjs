...
async function inspect() {
  if (typeof registerHooks !== "function" || process.execArgv.length || process.argv.length !== 4
    || !["inspect", "inspect-host", "inspect-database", "inspect-envfiles", "inspect-helpers", "inspect-retained-profile", "inspect-default-context", "inspect-pre32-host-pair", "inspect-pre32-host-pair-v5"].includes(process.argv[2]) || process.argv[3] !== "--json"
  ...
  try {
    ...
    let host, envFiles, helpers, retainedProfile, defaultContext, pre32HostPair, pre32HostPairV5;
    ...
    if (["inspect-pre32-host-pair", "inspect-pre32-host-pair-v5"].includes(process.argv[2])) {
      ...
      const observed = v5 ? await pairModule.observeCodeOwnedPositiveWorktreePre32HostPairV5()
      ...
      if (fields.schema !== (v5 ? "setfarm.internal-production-pre32-physical-database-pair.v5"
      ...
      if (v5) pre32HostPairV5 = observed;
    ...
    result = { schema: "setfarm.internal-production-deployment-cutover-bootstrap-observation.v1", sourceBuild, controllerSourceHash: authority.controllerSourceHash,
      ...
      ...(pre32HostPairV5 ? { pre32HostPairV5 } : {}) };
  } catch (error) {
    ...
    const pre32 = ["inspect-pre32-host-pair", "inspect-pre32-host-pair-v5"].includes(process.argv[2]) && refusal.stage === "pre32-host-pair";
...
try { process.stdout.write(`${JSON.stringify(await inspect())}\n`); }
  ...
  if (["inspect-default-context", "inspect-pre32-host-pair", "inspect-pre32-host-pair-v5"].includes(process.argv[2])) process.stderr.write(`${JSON.stringify({ schema: "setfarm.deployment-cutover-refusal.v1", ...refusal })}\n`);
  process.exitCode = 1;
}
