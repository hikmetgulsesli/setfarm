...
const MAX_SOURCE_BYTES_V2 = 4 * 1024 * 1024;
...
const sourcePath = path.join(
  repositoryRoot,
  "native/darwin/platform-release-bootstrap-suspended-exec-controller-fixture-v2.c",
);

function failure(message, code = "SUSPENDED_EXEC_FIXTURE_BUILD_FAILED") {
  const error = new Error(message.slice(0, 1_000));
  error.code = code;
  return error;
}

function fail(message, code = "SUSPENDED_EXEC_FIXTURE_BUILD_FAILED") {
...
function exactArgv(argv) {
  if (
    ...
    fail(
      "Usage: build-platform-release-bootstrap-suspended-exec-controller-fixture-v2.mjs --out-file ABSOLUTE_PATH",
    );
  }
  return argv[1];
}
...
function exactPrivateOutputParent(outFile) {
  ...
  if (
    ...
    fail(
      "Suspended-exec fixture output parent must be one external empty real process-owned mode-0700 directory",
    );
  }
  ...
  if (membershipErrors.length > 1) {
    throw new AggregateError(
      membershipErrors,
      "Suspended-exec fixture output parent membership and close both failed",
  ...
  try {
    lstatSync(outFile);
    fail("Suspended-exec fixture output must be absent");
  } catch (error) {
...
function captureOrdinaryFile(filePath, maxBytes = MAX_SOURCE_BYTES_V2) {
  ...
  try {
    ...
    if (
      ...
      fail("Suspended-exec fixture input is not one bounded ordinary file");
    }
    ...
    try {
      ...
      while (offset < byteLength) {
        ...
        if (count <= 0) {
          fail("Suspended-exec fixture input changed during capture");
        }
        offset += count;
      }
  ...
  if (captureErrors.length > 1) {
    throw new AggregateError(
      captureErrors,
      "Suspended-exec fixture input capture and close both failed",
...
function assertCaptureCurrent(filePath, expected, maxBytes = MAX_SOURCE_BYTES_V2) {
  ...
  try {
    if (
      ...
      fail("Suspended-exec fixture build input changed during compilation");
    }
  } finally {
...
function primaryFirstFailure(primary, containmentErrors, message) {
  ...
  aggregate.code = errors[0] instanceof Error && "code" in errors[0]
    ...
    : "SUSPENDED_EXEC_FIXTURE_BUILD_FAILED";
  throw aggregate;
}
...
async function runExact(
  ...
  if (
    ...
    fail("Suspended-exec fixture tool readiness contract is invalid");
  }
    ...
    try {
      ...
      if (!(error instanceof Error && "code" in error && error.code === "ESRCH")) {
        latchContainmentError(
          `Suspended-exec fixture tool ${path.basename(executable)} direct-child fallback kill failed`,
          "SUSPENDED_EXEC_FIXTURE_TOOL_CHILD_KILL_FAILED",
          error,
        );
      }
    }
  ...
  const armSettlementWatchdog = () => {
    ...
    settlementTimer = setTimeout(() => {
      const settlementFailure = failure(
        `Suspended-exec fixture tool ${path.basename(executable)} did not settle after termination`,
  ...
  const requestTermination = () => {
    ...
    try {
      if (testFault === "group_kill_failure_settlement_watchdog") {
        throw failure(
          "Injected suspended-exec fixture process-group kill failure",
          "SUSPENDED_EXEC_FIXTURE_TEST_INJECTED_GROUP_KILL_FAILURE",
        );
      }
      ...
      if (!(error instanceof Error && "code" in error && error.code === "ESRCH")) {
        latchContainmentError(
          `Suspended-exec fixture tool ${path.basename(executable)} process-group kill failed`,
  ...
  const armExecutionTimeout = () => {
    ...
    executionTimer = setTimeout(() => {
      latchFirstCause(
        failure(
          `Suspended-exec fixture tool ${path.basename(executable)} timed out after ${timeoutMilliseconds}ms`,
  ...
  const captureOutput = (name, chunk) => {
    ...
    if (currentByteLength + bytes.byteLength > MAX_TOOL_OUTPUT_BYTES_V2) {
      latchFirstCause(
        failure(
          `Suspended-exec fixture tool ${path.basename(executable)} exceeded the bounded ${name} capture`,
          "SUSPENDED_EXEC_FIXTURE_TOOL_OUTPUT_LIMIT",
        ),
        true,
      );
      return;
  ...
  const streamFailure = (name, error) => {
    const wrapped = failure(
      `Suspended-exec fixture tool ${path.basename(executable)} ${name} stream failed`,
      `SUSPENDED_EXEC_FIXTURE_TOOL_${name.toUpperCase()}_STREAM_FAILED`,
    );
    wrapped.cause = error;
  ...
  child.once("error", (error) => {
    const wrapped = failure(
      `Suspended-exec fixture tool ${path.basename(executable)} could not start`,
      "SUSPENDED_EXEC_FIXTURE_TOOL_SPAWN_FAILED",
    );
    wrapped.cause = error;
    latchFirstCause(wrapped, true);
  });
  ...
  if (testFault === "stdout_stream_error") {
    queueMicrotask(() => {
      child.stdout.destroy(new Error(
        "Injected suspended-exec fixture stdout stream failure",
      ));
    });
  } else if (testFault === "stderr_stream_error") {
    queueMicrotask(() => {
      child.stderr.destroy(new Error(
        "Injected suspended-exec fixture stderr stream failure",
      ));
    });
  }
  ...
  if (readinessToken === undefined) {
    ...
    readinessTimer = setTimeout(() => {
      latchFirstCause(
        failure(
          `Suspended-exec fixture tool ${path.basename(executable)} readiness was not observed`,
          "SUSPENDED_EXEC_FIXTURE_TOOL_READINESS_TIMEOUT",
        ),
        true,
      );
  ...
  if (
    ...
    firstCause = failure(
      `Suspended-exec fixture tool ${path.basename(executable)} exited before readiness`,
      "SUSPENDED_EXEC_FIXTURE_TOOL_READINESS_MISSING",
    );
  }
  ...
  if (
    ...
    firstCause = failure(
      `Suspended-exec fixture tool failed: ${(
  ...
  if (Number.isInteger(child.pid) && child.pid > 0 && processGroupAlive(child.pid)) {
    if (firstCause === undefined) {
      firstCause = failure(
        `Suspended-exec fixture tool ${path.basename(executable)} left its process group alive`,
        "SUSPENDED_EXEC_FIXTURE_TOOL_GROUP_OUTLIVED_LEADER",
      );
    }
    requestTermination();
  }
  if (
    ...
    containmentErrors.push(failure(
      `Suspended-exec fixture tool ${path.basename(executable)} process-group death was not proven by ESRCH`,
  ...
  primaryFirstFailure(
    firstCause,
    containmentErrors,
    "Suspended-exec fixture tool and containment both failed",
  );
...
export async function runSuspendedExecControllerFixtureRunnerFaultForTestV2(
  ...
  if (
    ...
    fail("Suspended-exec fixture runner test scenario is invalid");
  }
  await runExact(
    "/bin/sleep",
    [TEST_TIMEOUT_DURATION_SECONDS_V2],
    scenario === "stdout_stream_error" || scenario === "stderr_stream_error"
      ? TOOL_TIMEOUT_MILLISECONDS_V2
      : TEST_TIMEOUT_MILLISECONDS_V2,
    { testFault: scenario },
  );
  fail("Suspended-exec fixture runner test scenario did not fail closed");
}
...
export function runSuspendedExecControllerFixtureDescriptorSettlementFaultForTestV2(
  ...
  if (
    ...
    fail("Suspended-exec fixture descriptor test scenario is invalid");
  }
  ...
  runDescriptorOperationPrimaryFirst(
    ...
    "Suspended-exec fixture descriptor operation and close both failed",
    scenario === "close_failure"
      || scenario === "operation_and_close_failure"
      ? () => {
          throw closeError;
  ...
  fail("Suspended-exec fixture descriptor test scenario did not fail closed");
}
...
function syncDirectory(directory) {
  runDescriptorOperationPrimaryFirst(
    ...
    "Suspended-exec fixture directory fsync and descriptor close both failed",
  );
}
...
function syncFile(filePath) {
  runDescriptorOperationPrimaryFirst(
    ...
    (descriptor) => fsyncSync(descriptor),
    "Suspended-exec fixture file fsync and descriptor close both failed",
  );
}
  ...
  runDescriptorOperationPrimaryFirst(
    ...
    (descriptor) => {
      ...
      while (offset < bytes.byteLength) {
        ...
        if (count < 1) fail("Suspended-exec captured source write ended early");
        offset += count;
      }
      fsyncSync(descriptor);
    },
    "Suspended-exec captured source write and descriptor close both failed",
  );
}
...
function capturePinnedPublication(outFile, publicationPin, captured) {
  ...
  if (!samePublicationPin(beforeCapture, publicationPin)) {
    fail("Suspended-exec publication changed before its exact fence");
  }
  const published = captureOrdinaryFile(outFile);
  ...
  if (
    ...
    fail("Suspended-exec publication failed its exact stable-identity fence");
  }
  return published;
}
...
function publishCapturedBinary(outFile, outputParent, captured) {
  ...
  try {
    ...
    const forcedPartialFailure =
      process.env.SETFARM_SUSPENDED_EXEC_TEST_FORCE_PARTIAL_PUBLICATION_FAILURE_V2
        === "1";
    ...
    while (offset < publicationByteLength) {
      ...
      if (count < 1) fail("Suspended-exec publication write ended early");
      offset += count;
    }
    fchmodSync(descriptor, 0o500);
    fsyncSync(descriptor);
    if (forcedPartialFailure) {
      fail("Suspended-exec publication stopped after a forced partial write");
    }
    ...
    if (
      ...
      fail("Suspended-exec publication descriptor metadata is invalid");
    }
  } catch (error) {
  ...
  primaryFirstFailure(
    ...
    "Suspended-exec publication and durability close both failed",
  );

  const published = capturePinnedPublication(outFile, publicationPin, captured);
  return Object.freeze({ publicationPin, published });
}

export async function buildPlatformReleaseBootstrapSuspendedExecControllerFixtureV2(
  outFile,
) {
  if (process.platform !== "darwin") {
    fail("Suspended-exec fixture requires macOS");
  }
  ...
  const compileFlags = compileRecipeFlags.map((entry) =>
    entry === "MACOSX_SDK_V2" ? sdkPath : entry);
  const buildIdentity = {
    schema: "setfarm.platform-release-bootstrap-suspended-exec-controller-fixture-build-identity.v2",
    admissionScope: "test_fixture",
    credentialUse: "none",
    descriptorExecution: false,
    libprocApiStability: "private_unproven",
  ...
  const buildRecipeHash = sha256(Buffer.from(canonicalJson(buildIdentity), "utf8"));
  const stageRoot = mkdtempSync(
    path.join(outputParent, ".setfarm-suspended-exec-build-"),
  );
  chmodSync(stageRoot, 0o700);
  const stageSource = path.join(stageRoot, path.basename(sourcePath));
  const stageBinary = path.join(stageRoot, "suspended-exec-controller-v2");
  let captured;
  let published;
  try {
    writeCapturedSource(stageSource, source.bytes);
    syncDirectory(stageRoot);
    if (process.env.SETFARM_SUSPENDED_EXEC_TEST_FORCE_TOOL_TIMEOUT_V2 === "1") {
      await runExact(
        "/bin/sleep",
        [TEST_TIMEOUT_DURATION_SECONDS_V2],
        TEST_TIMEOUT_MILLISECONDS_V2,
    ...
    if (
      process.env.SETFARM_SUSPENDED_EXEC_TEST_FORCE_GRANDCHILD_TIMEOUT_V2
        === "1"
    ) {
      const marker = `setfarm-suspended-exec-grandchild-${path.basename(outputParent)}`;
      const readinessToken = `SETFARM_SUSPENDED_EXEC_GRANDCHILD_READY:${marker}`;
      await runExact(
        process.execPath,
        [
          "-e",
    ...
    if (
      process.env.SETFARM_SUSPENDED_EXEC_TEST_FORCE_TOOL_OUTPUT_LIMIT_V2
        === "1"
    ) {
      await runExact(
        process.execPath,
    ...
    if (process.env.SETFARM_SUSPENDED_EXEC_TEST_FORCE_TOOL_ERROR_V2 === "1") {
      await runExact("/usr/bin/false", []);
    }
    if (
      process.env
        .SETFARM_SUSPENDED_EXEC_TEST_FORCE_PARTIAL_COMPILER_OUTPUT_FAILURE_V2
        === "1"
    ) {
      await runExact(
        process.execPath,
    ...
    await runExact(clangPath, [...compileFlags, stageSource, "-o", stageBinary]);
    ...
    if (
      await runExact("/usr/bin/xcrun", ["--sdk", "macosx", "--show-sdk-path"])
        !== sdkPath
      ...
      fail("Suspended-exec fixture toolchain changed during compilation");
    }
    ...
    if (canonicalJson(architectureSet) !== canonicalJson(["arm64", "x86_64"])) {
      fail("Suspended-exec fixture lacks the exact universal architecture set");
    }
    chmodSync(stageBinary, 0o500);
    syncFile(stageBinary);
    syncDirectory(stageRoot);
    captured = captureOrdinaryFile(stageBinary);
    ...
    if (
      ...
      fail("Suspended-exec compiler output metadata is invalid");
    }
    if (
      process.env.SETFARM_SUSPENDED_EXEC_TEST_FORCE_POST_COMPILE_FAILURE_V2
        === "1"
    ) {
      fail("Suspended-exec fixture stopped after a forced post-compile failure");
    }
    if (
      process.env.SETFARM_SUSPENDED_EXEC_TEST_PAUSE_BEFORE_PUBLICATION_V2
        === "1"
    ) {
    ...
    if (
      process.env.SETFARM_SUSPENDED_EXEC_TEST_FORCE_POST_PUBLICATION_FAILURE_V2
        === "1"
    ) {
      fail("Suspended-exec fixture stopped after a forced post-publication failure");
    }
    ...
    const publishedStat = lstatSync(outFile);
    if (
      published.sha256 !== captured.sha256
      || published.byteLength !== captured.byteLength
      || publishedStat.nlink !== 1
      || publishedStat.uid !== process.getuid()
      || publishedStat.gid !== process.getgid()
      || (publishedStat.mode & 0o7777) !== 0o500
    ) {
      fail("Suspended-exec publication did not preserve compiled bytes");
    }
    return Object.freeze({
      schema: "setfarm.platform-release-bootstrap-suspended-exec-controller-fixture-build-receipt.v2",
      admissionScope: "test_fixture",
      ...
      buildRecipeHash,
      compileContract: Object.freeze({
        compileFlags: Object.freeze([...compileRecipeFlags]),
        deploymentTarget: DEPLOYMENT_TARGET_V2,
        frameworks: Object.freeze(["CoreFoundation", "Security"]),
        spawnFlags: Object.freeze([
          "POSIX_SPAWN_START_SUSPENDED",
          "POSIX_SPAWN_CLOEXEC_DEFAULT",
        ]),
      }),
...
async function main() {
  const receipt = await buildPlatformReleaseBootstrapSuspendedExecControllerFixtureV2(
...
if (process.argv[1] && realpathSync(process.argv[1]) === scriptPath) {
  main().catch((error) => {
    ...
    process.stderr.write(
      `[build-platform-release-bootstrap-suspended-exec-controller-fixture-v2] ${code}: ${
