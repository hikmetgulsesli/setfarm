...
const TEST_PUBLICATION_READINESS_TOKEN_V2 =
...
const sourcePaths = Object.freeze([
  path.join(nativeRoot, "platform-release-content-store-filesystem-kernel-v2.c"),
  path.join(nativeRoot, "platform-release-content-store-filesystem-fixture-v2.c"),
]);
...
function failure(message, code = "CONTENT_STORE_FIXTURE_BUILD_FAILED") {
  const error = new Error(message.slice(0, 1_000));
  error.code = code;
  return error;
}

function fail(message, code = "CONTENT_STORE_FIXTURE_BUILD_FAILED") {
...
function exactArgv(argv) {
  if (
    ...
    fail(
      "Usage: build-platform-release-content-store-filesystem-fixture-v2.mjs --out-file ABSOLUTE_PATH",
    );
  }
  return argv[1];
}
...
async function runExact(
  ...
  if (
    ...
    fail("Darwin content-store filesystem fixture tool readiness contract is invalid");
  }
    ...
    try {
      ...
      if (!(error instanceof Error && "code" in error && error.code === "ESRCH")) {
        latchContainmentError(
          `Darwin content-store filesystem fixture tool ${path.basename(executable)} direct-child fallback kill failed`,
          "CONTENT_STORE_FIXTURE_TOOL_CHILD_KILL_FAILED",
          error,
        );
      }
    }
  ...
  const armSettlementWatchdog = () => {
    ...
    settlementTimer = setTimeout(() => {
      const settlementFailure = failure(
        `Darwin content-store filesystem fixture tool ${path.basename(executable)} did not settle after termination`,
  ...
  const requestTermination = () => {
    ...
    try {
      if (testFault === "group_kill_failure_settlement_watchdog") {
        throw failure(
          "Injected Darwin content-store filesystem fixture process-group kill failure",
          "CONTENT_STORE_FIXTURE_TEST_INJECTED_GROUP_KILL_FAILURE",
        );
      }
      ...
      if (!(error instanceof Error && "code" in error && error.code === "ESRCH")) {
        latchContainmentError(
          `Darwin content-store filesystem fixture tool ${path.basename(executable)} process-group kill failed`,
  ...
  const armExecutionTimeout = () => {
    ...
    executionTimer = setTimeout(() => {
      latchFirstCause(
        failure(
          `Darwin content-store filesystem fixture tool ${path.basename(executable)} timed out after ${timeoutMilliseconds}ms`,
  ...
  const captureOutput = (name, chunk) => {
    if (outputLimitExceeded || firstCause !== undefined) return;
    ...
    if (currentByteLength + bytes.byteLength > MAX_TOOL_OUTPUT_BYTES_V2) {
      ...
      latchFirstCause(
        failure(
          `Darwin content-store filesystem fixture tool ${path.basename(executable)} exceeded the bounded ${name} capture`,
  ...
  const streamFailure = (name, error) => {
    const wrapped = failure(
      `Darwin content-store filesystem fixture tool ${path.basename(executable)} ${name} stream failed`,
      `CONTENT_STORE_FIXTURE_TOOL_${name.toUpperCase()}_STREAM_FAILED`,
    );
    wrapped.cause = error;
  ...
  child.once("error", (error) => {
    const wrapped = failure(
      `Darwin content-store filesystem fixture tool ${path.basename(executable)} could not start`,
      "CONTENT_STORE_FIXTURE_TOOL_SPAWN_FAILED",
    );
    wrapped.cause = error;
    latchFirstCause(wrapped, true);
  });
  ...
  if (testFault === "stdout_stream_error") {
    queueMicrotask(() => {
      child.stdout.destroy(new Error(
        "Injected Darwin content-store filesystem fixture stdout stream failure",
      ));
    });
  }
  ...
  if (readinessToken === undefined) {
    ...
    readinessTimer = setTimeout(() => {
      latchFirstCause(
        failure(
          `Darwin content-store filesystem fixture tool ${path.basename(executable)} readiness was not observed`,
          "CONTENT_STORE_FIXTURE_TOOL_READINESS_TIMEOUT",
        ),
        true,
      );
  ...
  if (
    ...
    firstCause = failure(
      `Darwin content-store filesystem fixture tool ${path.basename(executable)} exited before readiness`,
      "CONTENT_STORE_FIXTURE_TOOL_READINESS_MISSING",
    );
  }
  ...
  if (
    ...
    firstCause = failure(
      `Darwin content-store filesystem fixture tool failed: ${(
  ...
  if (Number.isInteger(child.pid) && child.pid > 0 && processGroupAlive(child.pid)) {
    if (firstCause === undefined) {
      firstCause = failure(
        `Darwin content-store filesystem fixture tool ${path.basename(executable)} left its process group alive`,
        "CONTENT_STORE_FIXTURE_TOOL_GROUP_OUTLIVED_LEADER",
      );
    }
    requestTermination();
  }
  if (
    ...
    containmentErrors.push(failure(
      `Darwin content-store filesystem fixture tool ${path.basename(executable)} process-group death was not proven by ESRCH`,
  ...
  primaryFirstFailure(
    firstCause,
    containmentErrors,
    "Darwin content-store filesystem fixture tool and containment both failed",
  );
  return Buffer.concat(stdoutChunks, stdoutByteLength).toString("utf8").trim();
}

export async function runContentStoreFilesystemFixtureRunnerFaultForTestV2(
  scenario,
) {
  if (
    ...
    fail("Darwin content-store filesystem fixture runner test scenario is invalid");
  }
  if (scenario === "multi_chunk_output_overflow") {
    await runExact(
      process.execPath,
      [
        "-e",
  ...
  fail("Darwin content-store filesystem fixture runner test scenario did not fail closed");
}
...
export function runContentStoreFilesystemFixtureDescriptorSettlementFaultForTestV2(
  ...
  if (
    ...
    fail("Darwin content-store filesystem fixture descriptor test scenario is invalid");
  }
  ...
  runDescriptorOperationPrimaryFirst(
    ...
    "Darwin content-store filesystem fixture descriptor operation and close both failed",
    scenario === "close_failure"
      || scenario === "operation_and_close_failure"
      ? () => {
          throw closeError;
  ...
  fail(
    "Darwin content-store filesystem fixture descriptor test scenario did not fail closed",
  );
}
...
function publishCapturedBinary(outFile, outputParent, captured) {
  ...
  try {
    ...
    const forcedPartialFailure =
      process.env
        .SETFARM_CONTENT_STORE_FIXTURE_TEST_FORCE_PARTIAL_PUBLICATION_FAILURE_V2
        === "1";
...
export async function buildPlatformReleaseContentStoreFilesystemFixtureV2(
  outFile,
) {
  if (process.platform !== "darwin") {
    fail("Darwin content-store filesystem native fixture requires macOS");
  }
  ...
  const buildIdentity = {
    schema:
      "setfarm.platform-release-content-store-filesystem-fixture-build-identity.v2",
  ...
  const stageRoot = mkdtempSync(
    path.join(outputParent, ".setfarm-content-store-fixture-build-"),
  );
  chmodSync(stageRoot, 0o700);
  const stageBinary = path.join(stageRoot, "content-store-filesystem-fixture-v2");
  let captured;
  let published;
  try {
    ...
    if (
      process.env.SETFARM_CONTENT_STORE_FIXTURE_TEST_FORCE_TOOL_TIMEOUT_V2
        === "1"
    ) {
      await runExact(
        "/bin/sleep",
    ...
    if (
      process.env
        .SETFARM_CONTENT_STORE_FIXTURE_TEST_FORCE_GRANDCHILD_TIMEOUT_V2
        === "1"
    ) {
      const marker = `setfarm-content-store-filesystem-grandchild-${path.basename(outputParent)}`;
      const readinessToken = `SETFARM_CONTENT_STORE_FILESYSTEM_GRANDCHILD_READY:${marker}`;
      await runExact(
        process.execPath,
        [
          "-e",
    ...
    if (
      process.env
        .SETFARM_CONTENT_STORE_FIXTURE_TEST_FORCE_TOOL_OUTPUT_LIMIT_V2
        === "1"
    ) {
      await runExact(
        process.execPath,
    ...
    if (
      process.env.SETFARM_CONTENT_STORE_FIXTURE_TEST_FORCE_TOOL_ERROR_V2
        === "1"
    ) {
    ...
    if (
      process.env
        .SETFARM_CONTENT_STORE_FIXTURE_TEST_FORCE_PARTIAL_COMPILER_OUTPUT_FAILURE_V2
        === "1"
    ) {
      await runExact(
        process.execPath,
    ...
    if (
      process.env
        .SETFARM_CONTENT_STORE_FIXTURE_TEST_FORCE_POST_COMPILE_FAILURE_V2
        === "1"
    ) {
    ...
    if (
      process.env
        .SETFARM_CONTENT_STORE_FIXTURE_TEST_PAUSE_BEFORE_PUBLICATION_V2
        === "1"
    ) {
      await new Promise((resolve, reject) => {
        process.stderr.write(
          `${TEST_PUBLICATION_READINESS_TOKEN_V2}\n`,
          (error) => {
            if (error) {
              const wrapped = failure(
                "Native fixture publication readiness handshake failed",
                "CONTENT_STORE_FIXTURE_PUBLICATION_READINESS_WRITE_FAILED",
              );
              wrapped.cause = error;
              reject(wrapped);
              return;
    ...
    if (
      process.env
        .SETFARM_CONTENT_STORE_FIXTURE_TEST_FORCE_POST_PUBLICATION_FAILURE_V2
        === "1"
    ) {
    ...
    return Object.freeze({
      schema:
        "setfarm.platform-release-content-store-filesystem-fixture-build-receipt.v2",
      admissionScope: "test_fixture",
...
async function main() {
  const receipt = await buildPlatformReleaseContentStoreFilesystemFixtureV2(
...
if (process.argv[1] && realpathSync(process.argv[1]) === scriptPath) {
  main().catch((error) => {
    const code = error instanceof Error && "code" in error
      ...
      : "CONTENT_STORE_FIXTURE_BUILD_FAILED";
    process.stderr.write(
      `[build-platform-release-content-store-filesystem-fixture-v2] ${code}: ${
