...
} from "../build-platform-release-bootstrap-darwin-host-self-observation-fixture-v2.mjs";

const repositoryRoot = join(dirname(fileURLToPath(import.meta.url)), "..", "..");
const BUILD_TIMEOUT_MILLISECONDS_V2 = 120_000;
const TEST_TIMEOUT_DURATION_SECONDS_V2 = "73.271828";
const STAGE_PREFIX_V2 = ".setfarm-host-self-observation-build-";
const STAGE_SOURCE_NAME_V2 =
  "platform-release-bootstrap-host-self-observation-fixture-v2.c";
const STAGE_BINARY_NAME_V2 = "host-self-observation-v2";
const builder = join(
  repositoryRoot,
  "scripts",
  "build-platform-release-bootstrap-darwin-host-self-observation-fixture-v2.mjs",
);
...
function assertRetainedWorkspaceSourceContract() {
  ...
  const runnerContracts = [
    ...
    /readinessToken/u,
    /DARWIN_HOST_SELF_OBSERVATION_FIXTURE_TOOL_READINESS_TIMEOUT/u,
  ...
  const publicationCall = source.indexOf(
    "const publication = publishCapturedBinary",
  );
  const finalFence = source.slice(
    publicationCall,
    source.indexOf("return Object.freeze({", publicationCall),
  );
  assert.match(
    finalFence,
...
function assertDescriptorSettlementSourceContract() {
  ...
  assert.match(
    seam,
    /runDarwinHostSelfObservationFixtureDescriptorSettlementFaultForTestV2\(\s*scenario,\s*\)/u,
  );
  assert.match(
    seam,
...
async function waitForRetainedStage(root) {
  for (let attempt = 0; attempt < 500; attempt += 1) {
    if (readdirSync(root).some((name) => name.startsWith(STAGE_PREFIX_V2))) return;
...
describe("Darwin host self-observation native fixture builder", () => {
  ...
  it("rejects invalid parents and retains a timed-out partial workspace", {
    ...
    try {
      ...
      assert.match(
        timedOut.stderr,
        /DARWIN_HOST_SELF_OBSERVATION_FIXTURE_TOOL_TIMEOUT:.*timed out/u,
      );
  ...
  it("retains tool, compiler, compiled, and publication failure evidence", {
    ...
    try {
      ...
      assert.match(
        ...
        /DARWIN_HOST_SELF_OBSERVATION_FIXTURE_TOOL_OUTPUT_LIMIT/u,
      );
      assertRetainedPartialStage(
        cases.find((entry) => entry.name === "output-limit").root,
      );
      assertRetainedPartialStage(
        cases.find((entry) => entry.name === "tool-error").root,
      );

      const partialCompiler = cases.find(
        (entry) => entry.name === "partial-compiler",
      );
      ...
      assert.equal(
        statSync(results.get("partial-publication").output).mode & 0o7777,
        0o500,
      );
  ...
  it("kills a readiness-proven timed-out tool's whole process group", {
    ...
    try {
      ...
      const marker = `setfarm-host-self-observation-grandchild-${basename(root)}`;
      ...
      assert.match(
        result.stderr,
        /DARWIN_HOST_SELF_OBSERVATION_FIXTURE_TOOL_TIMEOUT/u,
      );
  ...
  it("runtime-proves stream and bounded containment failure ordering", {
    ...
    assert.equal(
      streamError.code,
      "DARWIN_HOST_SELF_OBSERVATION_FIXTURE_TOOL_STDOUT_STREAM_FAILED",
    );
  ...
  it("preserves descriptor operation primacy and reports close failures", async () => {
    assert.equal(
      runDarwinHostSelfObservationFixtureDescriptorSettlementFaultForTestV2.length,
      1,
    );
    ...
    assert.equal(
      invalid.message,
      "Host self-observation fixture descriptor test scenario is invalid",
    );
