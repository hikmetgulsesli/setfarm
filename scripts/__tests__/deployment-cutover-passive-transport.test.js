...
function invoke(fault, identify = false) {
...
const request=${JSON.stringify(identify ? Object.fromEntries(["pid", "uid", "gid", "executable"].map(key => [key, request[key]])) : request)}, measured=${JSON.stringify(measured)}, fault=${JSON.stringify(fault)}, identify=${identify};
if(identify){measured.schema='setfarm.internal-production-passive-process-identity.v1';delete measured.homeContext;delete measured.completeEnvironmentValidated;delete measured.stableDoubleRead}
cp.spawnSync=(executable,args,options)=>{
  ...
  assert.deepEqual(options.env,{PATH:'/usr/bin:/bin',LANG:'C',LC_ALL:'C'});assert.deepEqual(JSON.parse(options.input.toString()),{...request,operation:identify?'identify':'measure'});
  ...
  if(fault==='generation')measured.startSeconds++;
...
try{const module=await import(${JSON.stringify(transport.href)});const value=module[identify?'identifyDeploymentCutoverPassiveProcessV1':'measureDeploymentCutoverPassiveHomeV1'](request);process.stdout.write(JSON.stringify({value,frozen:Object.isFrozen(value)}))}
...
test("identity-only transport returns a separate generation commitment", () => {
  ...
  assert.deepEqual(JSON.parse(invoke("none", true)), { value: identity, frozen: true });
});
