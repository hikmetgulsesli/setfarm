...
} from "../build-platform-release-bootstrap-darwin-filesystem-fixture-v2.mjs";

const repositoryRoot = join(dirname(fileURLToPath(import.meta.url)), "..", "..");
const BUILD_TIMEOUT_MILLISECONDS_V2 = 120_000;
const TEST_TIMEOUT_DURATION_SECONDS_V2 = "73.271828";
const STAGE_PREFIX_V2 = ".setfarm-native-fixture-build-";
const STAGE_SOURCE_NAMES_V2 = Object.freeze([
  "platform-release-bootstrap-filesystem-fixture-v2.c",
  "platform-release-bootstrap-filesystem-kernel-v2.c",
  "platform-release-bootstrap-filesystem-kernel-v2.h",
]);
const STAGE_BINARY_NAME_V2 = "darwin-filesystem-fixture-v2";
const builder = join(
  repositoryRoot,
  "scripts",
  "build-platform-release-bootstrap-darwin-filesystem-fixture-v2.mjs",
);
...
function assertRetainedWorkspaceSourceContract() {
  ...
  const runnerContracts = [
    ...
    /readinessToken/u,
    /DARWIN_FILESYSTEM_FIXTURE_TOOL_READINESS_TIMEOUT/u,
...
function assertDescriptorSettlementSourceContract() {
  ...
  assert.match(
    seam,
    /runDarwinFilesystemFixtureDescriptorSettlementFaultForTestV2\(\s*scenario,\s*\)/u,
  );
  assert.match(
    seam,
...
describe("Darwin filesystem backend native fixture builder", () => {
  ...
  it("rejects invalid parents and retains a timed-out partial workspace", {
    ...
    try {
      ...
      const timedOut = build(timeoutOutput, {
        SETFARM_DARWIN_FILESYSTEM_FIXTURE_TEST_FORCE_TOOL_TIMEOUT_V2: "1",
      });
      assert.equal(timedOut.status, 1);
      assert.equal(timedOut.signal, null);
      assert.match(
        timedOut.stderr,
        /DARWIN_FILESYSTEM_FIXTURE_TOOL_TIMEOUT:.*timed out/u,
      );
  ...
  it("retains tool, compiler, compiled, and publication failure evidence", {
    ...
    const cases = [
      ["output-limit", "SETFARM_DARWIN_FILESYSTEM_FIXTURE_TEST_FORCE_TOOL_OUTPUT_LIMIT_V2"],
      ["tool-error", "SETFARM_DARWIN_FILESYSTEM_FIXTURE_TEST_FORCE_TOOL_ERROR_V2"],
      ["partial-compiler", "SETFARM_DARWIN_FILESYSTEM_FIXTURE_TEST_FORCE_PARTIAL_COMPILER_OUTPUT_FAILURE_V2"],
      ["post-compile", "SETFARM_DARWIN_FILESYSTEM_FIXTURE_TEST_FORCE_POST_COMPILE_FAILURE_V2"],
    ...
    try {
      ...
      assert.match(
        ...
        /DARWIN_FILESYSTEM_FIXTURE_TOOL_OUTPUT_LIMIT/u,
      );
  ...
  it("kills a readiness-proven timed-out tool's whole process group", {
    ...
    try {
      ...
      const marker = `setfarm-darwin-filesystem-grandchild-${basename(root)}`;
      const result = build(output, {
        SETFARM_DARWIN_FILESYSTEM_FIXTURE_TEST_FORCE_GRANDCHILD_TIMEOUT_V2: "1",
      });
      assert.equal(result.status, 1);
      assert.match(result.stderr, /DARWIN_FILESYSTEM_FIXTURE_TOOL_TIMEOUT/u);
  ...
  it("preserves a foreign output that wins the O_EXCL publication race", {
    ...
    try {
      ...
      running = buildAsync(output, {
        SETFARM_DARWIN_FILESYSTEM_FIXTURE_TEST_PAUSE_BEFORE_PUBLICATION_V2: "1",
      });
  ...
  it("runtime-proves stream and bounded containment failure ordering", {
    ...
    const streamError = await captureFailure(() =>
      runDarwinFilesystemFixtureRunnerFaultForTestV2("stdout_stream_error"));
    assert.equal(
      streamError.code,
      "DARWIN_FILESYSTEM_FIXTURE_TOOL_STDOUT_STREAM_FAILED",
    );
  ...
  it("preserves descriptor operation primacy and reports close failures", async () => {
    assert.equal(
      runDarwinFilesystemFixtureDescriptorSettlementFaultForTestV2.length,
      1,
    );
    const invalid = await captureFailure(() =>
      runDarwinFilesystemFixtureDescriptorSettlementFaultForTestV2("invalid"));
    assert.equal(invalid.code, "DARWIN_FILESYSTEM_FIXTURE_BUILD_FAILED");
    assert.equal(
      invalid.message,
      "Darwin filesystem fixture descriptor test scenario is invalid",
    );
